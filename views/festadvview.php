...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['hdaction'])) {
        ...
        if (isset($_POST['empnotxt'])) {
            ...
            $famttxt = mysql_escape_string($_POST['famttxt']);
            $noitxt = mysql_escape_string($_POST['noitxt']);
            $finstamttxt = mysql_escape_string($_POST['finstamttxt']);
            ...
            if ($_POST['hdaction'] == "save") {
                $qry = "INSERT INTO festival_adv(emp_no,festamt,balance_due,total_inst,remaining_inst,loan_date,from_date,to_date,inst_amt)
             VALUES('$empnotxt',$famttxt,$famttxt,$noitxt,$noitxt,'$date','$fdate','$tdate',$finstamttxt);";
            } else {

                $qry = "update festival_adv set balance_due=$famttxt,remaining_inst=$noitxt,inst_amt=$finstamttxt,from_date='$fdate',to_date='$tdate' where emp_no='$empnotxt';";
            }
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['mode']) && $_POST['mode'] == "edit") {
        ...
        if (isset($_POST['id'])) {
            ...
            $emid = $_POST['id'];
            $output = array();
            $qury = "SELECT * FROM festival_adv as f INNER JOIN emp_master as e ON e.emp_no = f.emp_no WHERE f.emp_no='" . $emid . "';";
...
if (isset($_POST['mode']) && $_POST['mode'] == "del") {
    ...
    if (isset($_POST['id'])) {
        ...
        $qury = "DELETE FROM festival_adv WHERE emp_no='" . $emid . "';";
